...
- Tests in `test/` use **inline fixtures** (`CLAUDE.md`, *Code Style*). A test must never walk
  ...
  [ "$total" -ge "${EXPECTED_ARTIFACTS:-87}" ] || { echo "SWEEP DID NOT SEE THE VAULT: $total"; exit 1; }
  ```
  ...
  **87** of 88 `.md` files are artifacts (the odd one out is `CoderByte/Tests/README.md`,
  correctly excluded) — 65 `function`, 14 `package`, 8 `stack`. **Raise the default in the same
  ...
  **Eight of the 87 are `stack` artifacts and print `SKIP … — stack, LEET_STACK_E2E not set`,
  ...
  minutes plus a network: seven of the eight install real ecosystems and boot real servers.
  The eighth, `CoderByte/Tests/stack/hello-stack.md`, is deliberately **offline and seconds long**
