 ...
 *
 * **Re-taken a third time, T3.5 (wave 3.D)** — the narrowing D12 anticipated.
 * Two byte-runs moved in each capture, and both are the same change seen twice:
 *
 * 1. `"test":{"type":…}` — `function`, `project` and `service` are no longer
 *    `TestTypeId` members, so `parseTestType` collapses each to
 *    `DEFAULT_TEST_TYPE`, now `"call"`. The *shape* these fixtures derive is
 *    unchanged (`leetcodeType` still reads `function`/`package`/`stack`), which
 *    is the C3 property: the derivation reads the **raw** scalar, this field
 *    reads the parsed one, and they now visibly disagree.
 * 2. The service fixture's warning text — its `http` check is no longer
 *    *unimplemented* (T3.5 dispatches it); it is dropped because it binds with
 *    the pre-rename `service: api` instead of `package: api`. The vault
 *    migrator rewrites that line; these fixtures are deliberately kept in their
 *    pre-migration form, so the capture records the drop.
 *
 * Nothing else in any capture moved.
 */
suite('migrate-artifact-format', () => {
	...
	test('a build-only project with no ## Tests anchors on ## Files instead', () => {
		...
		// The v1 `type: project` scalar survives the v1→v2 move into the fence;
		// what it *parses* to is `DEFAULT_TEST_TYPE` since T3.5 retired the
		// legacy ids. The shape it derives is the assertion that still bites.
		assert.strictEqual(parseLeetCode(out).test.type, 'call');
		assert.strictEqual(parseLeetCode(out).leetcodeType, 'package');
	});
