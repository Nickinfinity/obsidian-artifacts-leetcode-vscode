...
| TA.2b Tetris java blind-solve (G2) | A2 | — | blocked | — | — | needs an **independent** dispatch (a solver that never saw `# Solutions`); the orchestrator has now read the reference and is disqualified. Awaiting human go-ahead to spawn a worker |

## Phase B — `project` / react env (G1)

| Task | Wave | Owner | Status | Test count | Gate | Notes |
|------|------|-------|--------|-----------|------|-------|
| HG-B decisions 1,2,4,5 | HG-B | — | todo | — | — | human gate; dec.2 now zero-or-one function check; dec.3 already ships (no hunk); + doc-sync; 7,8 pre-resolved (confirm) |
...
| A2 | TA.2b dispatch | Orchestrator read `# Solutions` while authoring TA.2a → cannot be the blind solver. Requires a separate worker dispatch; **human go-ahead pending** |
| _pending HG-B_ | Case→check binding (spike §1) | rec: widen `JSON_FENCE` for `check=<name>`, take all fences |
| _pending HG-B_ | Per-check params/returns (spike §2) | rec: **zero-or-one** function-check per project; params/returns optional; project skips the function structural floor |
| _pending HG-B_ | `role: readonly` (spike §7) | rec: read-only file mode |
| _pending HG-B_ | JSX/TSX bundler (spike §6) | rec: per-run esbuild |
