 ...
 *
 * An env emits its own driver and calls `functionNameFor(parsed, langId)`
 * directly, so the candidate must be a bare declaration of that name — never a
 * program with its own `main`, and never a stdin reader. Three shapes arrive
 * here:
 *
 ...
 * @returns Source declaring `functionNameFor(parsed, langId)` and nothing else.
 *
 * @example
...
export function buildExecutable(parsed: ParsedLeetCode, langId: string, code: string): string {
	...
	if (declaresFunction(code, functionNameFor(parsed, langId))) { return code; }
 ...
 * @returns A declaration of `functionNameFor(parsed, langId)`, or the body
 *   unchanged for a language with no known declaration syntax.
 *
 * @example
...
function wrapBareBody(parsed: ParsedLeetCode, langId: string, body: string): string {
	...
	if (langId === 'java') {
		...
		return `\tpublic static ${ret} ${fn}(${params}) {\n\t\t${body}\n\t}`;
	}
	return body;
}
